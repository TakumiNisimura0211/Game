...
void Animation::PlayAnimation(int animationSetIndex,bool loop)
{
...
void Animation::PlayAnimation(int animationSetIndex, float interpolateTime, bool loop)
{
	if (animationSetIndex < numAnimSet) {
		if (pAnimController) {
			//補間開始の印。
...
void Animation::Update(float deltaTime)
{
	if (pAnimController && !endAnim) {
		...
		if (animationEndTime[currentAnimationSetNo] > 0.0 //アニメーションの終了時間が設定されている。
			...
			if (!loopAnim && animationSets[currentAnimationSetNo]->GetPeriod()<localAnimationTime){
				endAnim = true;
			}
			else{
				//普通に再生。
