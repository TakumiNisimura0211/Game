...
public:
	...
	*/
	void PlayAnimation(int animationSetIndex,bool loop);
	/*!
	*@brief	アニメーションの再生。アニメーションの補完が行われます。
	*@param[in]		animationSetIndex	再生したいアニメーションのインデックス。
	*@param[in]		interpolateTime		補間時間。
	*/
	void PlayAnimation(int animationSetIndex, float interpolateTime,bool loop);
#if 0
	/*!
...
private:
	...
	bool									loopAnim;				//!<今再生中のアニメーションをループさせるかのフラグ
	bool									endAnim;				//!<アニメーションが終了したかどうか
